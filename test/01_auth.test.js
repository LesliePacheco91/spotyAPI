...
const { testAuthRegister, testAuthLogin, unregisteredUser} = require("./helper/helperData");
...
test("deberia retornar un 201", async() => {
...
test("esto deberia de retornar 403 No existe el usuario", async () => {
    const response = await request(app)
    .post("/api/auth/login")
